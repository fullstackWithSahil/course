...
export async function POST(request: Request) {
	try {
		...
		for (const key of Object.keys(req.variables)) {
			...
			formatedHtml = formatedHtml.replace(`{{${key}}}`, req.variables[key]);
...
const requestSchema = z.object({
	to: z.array(nonEmptyString.email()).min(1, { message: '"to" array must not be empty.' }),
	subject: nonEmptyString,
	from: nonEmptyString.email(),
	template: nonEmptyString,
	variables: z.record(z.any()).refine(
		val => Object.keys(val).length > 0,
		{ message: '"variables" must be a non-empty object.' }
	),
});

export function schemaValidation(body:any) {
